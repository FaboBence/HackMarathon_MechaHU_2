...
for i in range(100000):
    tmp=random_schedule(np.full((df_rooms.shape[0],WORK_FINISH-WORK_BEGIN),-1),Friday_matrix)
    #print(tmp)
    pen=penalty_calculator(Friday_matrix,tmp)
    if pen==0:
        print(tmp)
        print(pen)
        print(i)
        ...
        tmp=fine_search(Friday_matrix,tmp)
        if penalty_calculator(Friday_matrix,tmp)==0:
             print(tmp)
             print(pen)
             print(i)
             print("Done by fine search")
             print()
...
print()
print("This is your weekly schedule!")
"""
for i,day in enumerate(Week_schedule):
    print()
    print(day)
    pen=penalty_calculator(day,i)
    print(pen)

    min_pen=1000
for i in range(10000):
    tmp=np.random.randint(-1,4,(df_rooms.shape[0],WORK_FINISH-WORK_BEGIN))
    pen=penalty_calculator(tmp,4)
    if i%1000==0:
        print('!!!')
    if pen<min_pen:
        min_pen=pen
print("Minpen: "+str(min_pen))
"""
